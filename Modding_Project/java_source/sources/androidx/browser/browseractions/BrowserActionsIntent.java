    ...
    public static List<BrowserActionItem> parseBrowserActionItems(@NonNull ArrayList<Bundle> arrayList) {
        ...
        for (int i10 = 0; i10 < arrayList.size(); i10++) {
            ...
            if (!TextUtils.isEmpty(string) && pendingIntent != null) {
                if (i11 != 0) {
                    browserActionItem = new BrowserActionItem(string, pendingIntent, i11);
                } else {
    ...
    static void launchIntent(Context context, Intent intent, List<ResolveInfo> list) {
        if (list != null && list.size() != 0) {
            ...
            if (list.size() == 1) {
                ...
                ResolveInfo resolveActivity = context.getPackageManager().resolveActivity(new Intent(CommonConstant.ACTION.HWID_SCHEME_URL, Uri.parse(TEST_URL)), 65536);
                if (resolveActivity != null) {
                    String str = resolveActivity.activityInfo.packageName;
                    while (true) {
                        if (i10 >= list.size()) {
                            break;
                        } else if (str.equals(list.get(i10).activityInfo.packageName)) {
