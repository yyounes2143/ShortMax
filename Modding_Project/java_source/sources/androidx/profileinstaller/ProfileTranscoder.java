...
public class ProfileTranscoder {
    ...
    private static int readFlagsFromBitmap(@NonNull BitSet bitSet, int i10, int i11) {
        int i12 = 2;
        if (!bitSet.get(methodFlagBitmapIndex(2, i10, i11))) {
            i12 = 0;
        }
        if (bitSet.get(methodFlagBitmapIndex(4, i10, i11))) {
            return i12 | 4;
        }
        return i12;
    }
    ...
    private static DexProfileData[] readUncompressedBody(@NonNull InputStream inputStream, @NonNull String str, int i10) throws IOException {
        ...
        for (int i11 = 0; i11 < i10; i11++) {
            ...
            dexProfileDataArr[i11] = new DexProfileData(str, Encoding.readString(inputStream, readUInt16), Encoding.readUInt32(inputStream), 0L, readUInt162, (int) Encoding.readUInt32(inputStream), (int) Encoding.readUInt32(inputStream), new int[readUInt162], new TreeMap());
        }
        for (int i12 = 0; i12 < i10; i12++) {
            DexProfileData dexProfileData = dexProfileDataArr[i12];
            ...
            for (int i12 = 1; i12 <= 4; i12 <<= 1) {
                if (i12 != 1 && (i12 & i10) != 0) {
                    if ((i12 & intValue2) == i12) {
                        int i13 = (dexProfileData.numMethodIds * i11) + intValue;
                        int i14 = i13 / 8;
