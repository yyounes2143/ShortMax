...
public class C1748Tw {
     ...
     */
    public static C1748Tw A04(File file) throws IOException {
        try {
            ...
            if (file.length() >= A00()) {
                ...
                if (readLong == -360011992771067903L) {
                    ...
                    if (randomAccessFile.length() >= A02((int) readLong2)) {
                        ...
                        return new C1748Tw(randomAccessFile, jArr, randomAccessFile.length());
                    }
    ...
    public C1748Tw(RandomAccessFile randomAccessFile, long[] jArr, long j10) throws IOException {
    ...
    public static C1748Tw A03(File file) throws IOException {
        RandomAccessFile randomAccessFile = new RandomAccessFile(file, A05(416, 3, 33));
        try {
            randomAccessFile.writeLong(-360011992771067903L);
            randomAccessFile.writeLong(C1749Tx.A03);
            randomAccessFile.write(new byte[C1749Tx.A03 * 8]);
            randomAccessFile.getFD().sync();
            return new C1748Tw(randomAccessFile, new long[C1749Tx.A03], randomAccessFile.length());
        } catch (IOException unused) {
