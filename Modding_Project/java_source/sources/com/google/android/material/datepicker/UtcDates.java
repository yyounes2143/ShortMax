...
class UtcDates {
    ...
    public static String getDefaultTextInputHint(Resources resources, SimpleDateFormat simpleDateFormat) {
        ...
        if (Locale.getDefault().getLanguage().equals(Locale.KOREAN.getLanguage())) {
            pattern = pattern.replaceAll("d+", "d").replaceAll("M+", "M").replaceAll("y+", TextureRenderKeys.KEY_IS_Y);
        }
        return pattern.replace("d", string3).replace("M", string2).replace(TextureRenderKeys.KEY_IS_Y, string);
    }
