...
public class MaxSegmentCollectionImpl extends MaxSegmentCollection {
    ...
    public String toString() {
        return "MaxSegmentColletionImpl{segments=" + this.f8540a + "}";
    }

    private MaxSegmentCollectionImpl(BuilderImpl builderImpl) {
        List<MaxSegment> list = builderImpl.f8542a;
        this.f8540a = list;
        this.f8541b = new HashMap();
        for (MaxSegment maxSegment : list) {
            this.f8541b.put("segment_" + maxSegment.getKey(), maxSegment.getValues());
        }
    }
}
