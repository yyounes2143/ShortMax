...
class FindFileFromCache implements IDownloadTask {
    ...
    private boolean checkDownloadReadyRate(long j10) {
        if (Utils.getDownloadRate(this._downloadModel.getTotalBytes(), j10) >= this._downloadMessage.getDownloadRate()) {
            return true;
        }
        return false;
    }
    ...
    private boolean checkUseFileResume() {
        ...
        builder.add(DownloadCommon.DOWNLOAD_REPORT_FIND_FILE_RESULT, DownloadCommon.DOWNLOAD_REPORT_FIND_FILE_RESULT_VALUE_C + z10);
        return z10;
    }
    ...
    public DownloadResponse run() {
        ...
        this._downloaderReporter.add("fts", String.valueOf(this._downloadModel.getTotalBytes()));
