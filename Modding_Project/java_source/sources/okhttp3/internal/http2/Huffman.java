...
public final class Huffman {
    ...
    private static final byte[] f64083c;
    @NotNull

    /* renamed from: a  reason: collision with root package name */
    public static final Huffman f64081a = new Huffman();
    @NotNull

    /* renamed from: b  reason: collision with root package name */
    private static final int[] f64082b = {8184, 8388568, 268435426, 268435427, 268435428, 268435429, 268435430, 268435431, 268435432, 16777194, 1073741820, 268435433, 268435434, 1073741821, 268435435, 268435436, 268435437, 268435438, 268435439, 268435440, 268435441, 268435442, 1073741822, 268435443, 268435444, 268435445, 268435446, 268435447, 268435448, 268435449, 268435450, 268435451, 20, 1016, 1017, 4090, 8185, 21, 248, 2042, 1018, 1019, 249, 2043, 250, 22, 23, 24, 0, 1, 2, 25, 26, 27, 28, 29, 30, 31, 92, 251, 32764, 32, 4091, 1020, 8186, 33, 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 252, 115, 253, 8187, 524272, 8188, 16380, 34, 32765, 3, 35, 4, 36, 5, 37, 38, 39, 6, 116, 117, 40, 41, 42, 7, 43, 118, 44, 8, 9, 45, 119, 120, 121, 122, 123, 32766, 2044, 16381, 8189, 268435452, 1048550, 4194258, 1048551, 1048552, 4194259, 4194260, 4194261, 8388569, 4194262, 8388570, 8388571, 8388572, 8388573, 8388574, 16777195, 8388575, 16777196, 16777197, 4194263, 8388576, 16777198, 8388577, 8388578, 8388579, 8388580, 2097116, 4194264, 8388581, 4194265, 8388582, 8388583, 16777199, 4194266, 2097117, 1048553, 4194267, 4194268, 8388584, 8388585, 2097118, 8388586, 4194269, 4194270, 16777200, 2097119, 4194271, 8388587, 8388588, 2097120, 2097121, 4194272, 2097122, 8388589, 4194273, 8388590, 8388591, 1048554, 4194274, 4194275, 4194276, 8388592, 4194277, 4194278, 8388593, 67108832, 67108833, 1048555, 524273, 4194279, 8388594, 4194280, 33554412, 67108834, 67108835, 67108836, 134217694, 134217695, 67108837, 16777201, 33554413, 524274, 2097123, 67108838, 134217696, 134217697, 67108839, 134217698, 16777202, 2097124, 2097125, 67108840, 67108841, 268435453, 134217699, 134217700, 134217701, 1048556, 16777203, 1048557, 2097126, 4194281, 2097127, 2097128, 8388595, 4194282, 4194283, 33554414, 33554415, 16777204, 16777205, 67108842, 8388596, 67108843, 134217702, 67108844, 67108845, 134217703, 134217704, 134217705, 134217706, 134217707, 268435454, 134217708, 134217709, 134217710, 134217711, 134217712, 67108846};
    @NotNull

    /* renamed from: d  reason: collision with root package name */
    private static final Node f64084d = new Node();
    ...
    static {
        ...
        for (int i10 = 0; i10 < length; i10++) {
            f64081a.a(i10, f64082b[i10], f64083c[i10]);
        }
    }
    ...
    private final void a(int i10, int i11, int i12) {
        ...
        n.C(a11, node, i15, (1 << i14) + i15);
    }

    public final void b(@NotNull BufferedSource source, long j10, @NotNull BufferedSink sink) {
        ...
        for (long j11 = 0; j11 < j10; j11++) {
            i10 = (i10 << 8) | Util.d(source.readByte(), 255);
            i11 += 8;
            while (i11 >= 8) {
                Node[] a10 = node.a();
                Intrinsics.checkNotNull(a10);
                node = a10[(i10 >>> (i11 - 8)) & 255];
        ...
        while (i11 > 0) {
            Node[] a11 = node.a();
            Intrinsics.checkNotNull(a11);
            Node node2 = a11[(i10 << (8 - i11)) & 255];
            Intrinsics.checkNotNull(node2);
            if (node2.a() == null && node2.c() <= i11) {
    ...
    public final void c(@NotNull ByteString source, @NotNull BufferedSink sink) throws IOException {
        Intrinsics.checkNotNullParameter(source, "source");
        Intrinsics.checkNotNullParameter(sink, "sink");
        ...
        for (int i11 = 0; i11 < size; i11++) {
            int d10 = Util.d(source.getByte(i11), 255);
            int i12 = f64082b[d10];
            byte b10 = f64083c[d10];
            j10 = (j10 << b10) | i12;
            i10 += b10;
            while (i10 >= 8) {
                i10 = (i10 == true ? 1 : 0) - 8;
    ...
    public final int d(@NotNull ByteString bytes) {
        Intrinsics.checkNotNullParameter(bytes, "bytes");
        int size = bytes.size();
        long j10 = 0;
        for (int i10 = 0; i10 < size; i10++) {
            j10 += f64083c[Util.d(bytes.getByte(i10), 255)];
        }
        return (int) ((j10 + 7) >> 3);
    }
